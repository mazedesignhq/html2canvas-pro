...
import { type CSSValue, isIdentToken, isNumberToken, nonFunctionArgSeparator, Parser } from '../syntax/parser';
import { type HashToken, TokenType } from '../syntax/tokenizer';
...
const _color = (context: Context, args: CSSValue[]) => {
  ...
  if (is_absolute) {
    ...
    let from = tokens[1].type === TokenType.FUNCTION ? tokens[1].values : isIdentToken(tokens[1]) ? [tokens[1]] : [];
